...
**Status: partially superseded by [ADR 0004](./0004-plain-staunton-pieces.md).** The fixed board
stands and is not in question. The orientation-neutral pieces did not survive contact with a
prototype and have been replaced by plain one-way Staunton pieces; the reasoning below is kept
because it explains why the alternative was attempted and what it cost to abandon it.
...
Five candidate glyph systems were prototyped (`prototype/piece-set` branch): top-down
realism, mirrored Staunton, movement semantics, a diagonal mirror, and plain one-way
Staunton as a control. Viewed side by side, **none of the four orientation-neutral systems
was liked**, and the plain Staunton control won outright. See
[ADR 0004](./0004-plain-staunton-pieces.md) for the decision that replaces this one.

The prototype earned its keep even so: it converted a confident paper assumption into a
tested one for the price of a single HTML file, and it showed that the knight is the piece
that decides any symmetric scheme, being the only one that faces a direction.
